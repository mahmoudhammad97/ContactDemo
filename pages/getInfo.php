		...
		style="background: -webkit-linear-gradient(left, black, darkslategray);position: relative;width: 100%;height: 100%;padding: 0 50px 60px 50px;">
			...
			<div class="list-group" id="newNumber" style="display:table;border:0px">
				...
				style="cursor:pointer;text-align: center;width: fit-content;margin-left: auto;padding:0px" onclick="inputNumber()">
					<img src = "../img/plus-square.svg" alt="My Happy SVG" style="width:40px"/>
				</div>
			</div>
		</div>
<script>
	...
	function show_number(){
		var xhttp = new XMLHttpRequest();
			xhttp.open("POST", "../CRUD/show.php", true);
	...
	{	
		...
		{	
			$(".list-group").append(
		"<div id=\""+i+"\">"+
			"<img src=\"../img/trash-2.svg\" style=\"float:right;cursor:pointer\" onclick=\"delete_num(this.parentNode.textContent)\"/>"+
			"<img src=\"../img/refresh-cw.svg\" style=\"float:right;margin-right:20px;cursor:pointer\" onclick=\"update_num(this.parentNode.textContent)\"/>"+
		"</div>");
	...
	function addNew(){
		...
		var request = [getId , input_num];  //[id of this contact , new number of contact]
			var xhttp = new XMLHttpRequest();
			xhttp.open("POST", "../CRUD/addExist.php", true);
			xhttp.onreadystatechange = function ()
			{
				if(xhttp.readyState === XMLHttpRequest.DONE) {
	...
	function update_num(value)
	{
		...
		var xhttp = new XMLHttpRequest();
			xhttp.open("POST", "../CRUD/update.php", true);
			xhttp.onreadystatechange = function ()
			{
				if(xhttp.readyState === XMLHttpRequest.DONE) {
					var myresponse = xhttp.responseText;
	...
	function delete_num(value)
	{
		...
		{
			var xhttp = new XMLHttpRequest();
				xhttp.open("POST", "../CRUD/delete.php", true);
				xhttp.onreadystatechange = function ()
				{
					if(xhttp.readyState === XMLHttpRequest.DONE) {
						var myresponse = xhttp.responseText;
